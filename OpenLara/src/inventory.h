...
*/
    ...
    void update() {
        ...
        if (page == PAGE_LEVEL_STATS) {
            ...
            if (!chosen) {
                ...
                switch (key) {
                    case cLeft  : { if (count > 0) { phaseSelect = 0.0f; targetIndex = (targetIndex - 1 + count) % count; } } break;
                    case cRight : { if (count > 0) { phaseSelect = 0.0f; targetIndex = (targetIndex + 1) % count;         } } break;
    ...
    void renderBackground(int view) {
        ...
        if (background[0] && background[0]->origHeight > 0 && background[0]->origWidth / background[0]->origHeight == 2) // PSX images aspect correction
